...
export default function ModuloFinanzas() {
  ...
  const cargarDatosFinancieros = async () => {
    ...
    try {
      ...
      const { data: quinielasDB, error: errQuinielas } = await supabase
        ...
        .order('fecha_cierre', { ascending: false }) // ✅ CORRECCIÓN APLICADA AQUÍ
      ...
      if (quinielasDB) {
        for (const q of quinielasDB) {
          // 🔥 MÉTODO SEGURO: Buscamos solo los IDs para contar rápido sin que Supabase colapse
  ...
  return (
    <div className="animate-in fade-in duration-500 w-full max-w-5xl mx-auto space-y-6">
      ...
      {/* 🔥 ALERTA DE ERROR SI FALLA ALGO EN SUPABASE */}
      {mensajeError && (

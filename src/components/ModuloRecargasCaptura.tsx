    ...
    <>
                                ...
                                return cajero.datosHistorial.map((mov: any) => {
                                  ...
                                  return (
                                    <tr key={mov.id} className="hover:bg-slate-800/30 transition-colors">
                                      ...
                                      <td className={`p-2 text-center font-black text-[10px] md:text-xs ${mov.cantidad > 0 ? 'text-green-500' : mov.cantidad < 0 ? 'text-red-500' : 'text-amber-500'}`}>
                                        {mov.cantidad > 0 ? '+' : (mov.cantidad === 0 ? '' : '-')}${Math.abs(mov.cantidad).toLocaleString('es-MX', {minimumFractionDigits:0})}
                                      </td>
                                      <td className="p-2 text-right font-black text-[10px] md:text-xs text-amber-400">
                                        ${saldoEnEseMomento.toLocaleString('es-MX', {minimumFractionDigits:0})}
                                      </td>
                                    </tr>
                                  )
                                })
      ...
      {vista === 'captura' && (
        <div className="animate-in fade-in duration-300 w-full max-w-3xl mx-auto">
          ...
          {(quinielasCapturables.length > 1 || (quinielasCapturables.length === 1 && captura.quiniela?.id !== quinielasCapturables[0].id)) && (
            <div className="flex flex-wrap justify-center gap-1.5 mb-4 bg-slate-900/50 p-2 rounded-xl border border-slate-800">
              {quinielasCapturables.map(qa => (
                <button key={qa.id} onClick={() => captura.seleccionarQuiniela(qa)} className={`px-3 py-1.5 rounded-lg text-[10px] md:text-xs font-black uppercase transition-all flex items-center gap-1.5 ${captura.quiniela?.id === qa.id ? 'bg-amber-500 text-slate-900 shadow-[0_0_10px_rgba(245,158,11,0.3)]' : 'bg-slate-950 border border-slate-700 text-slate-500 hover:text-slate-300'}`}>
                  {qa.modalidad === 'marcador_exacto' && <span className="text-[8px] bg-slate-900 text-amber-500 px-1 rounded">EX</span>}
                  {qa.modalidad === 'sorteo' && <span className="text-[8px] bg-slate-900 text-blue-500 px-1 rounded">🎲</span>}
                  {qa.nombre_jornada}
                </button>
              ))}
            </div>
          ...
          {quinielasCapturables.length === 0 ? (
            ...
            <div className={`bg-gradient-to-br ${esSorteo ? 'from-blue-950/30 to-slate-900 border-blue-900/40' : 'from-amber-950/20 to-slate-900 border-amber-900/40'} border rounded-2xl p-4 md:p-6 shadow-[0_0_20px_rgba(245,158,11,0.05)] relative overflow-hidden`}>
              
              <div className="mb-5 border-b border-amber-900/30 pb-3 flex flex-col md:flex-row md:items-center justify-between gap-2">
                <div>
                  <h3 className={`${esSorteo ? 'text-blue-400' : 'text-amber-500'} font-black uppercase tracking-widest text-sm md:text-base flex items-center gap-2`}>
                    <span>{esSorteo ? '🎲' : '⚡'}</span> Captura Física
                    {esMarcadorExacto && <span className="bg-amber-500/20 text-amber-400 border border-amber-500/50 px-2 py-0.5 rounded text-[8px] tracking-widest">MARCADOR EXACTO</span>}
                    {esSorteo && <span className="bg-blue-500/20 text-blue-400 border border-blue-500/50 px-2 py-0.5 rounded text-[8px] tracking-widest">SORTEO MUNDIAL</span>}
                  </h3>
                ...
                {esSorteo && (
                  <div className="bg-blue-950/50 border border-blue-900 px-3 py-1.5 rounded-lg text-right">
                    ...
                    <p className={`font-black text-lg leading-none ${captura.ocupacionSorteo >= 8 ? 'text-red-500' : 'text-white'}`}>
                      {captura.ocupacionSorteo} <span className="text-sm text-slate-500">/ 8</span>
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5 bg-slate-950/40 p-3 rounded-xl border border-slate-800/50 relative">
                <div className="relative">
                  <label className={`text-[9px] md:text-[10px] ${esSorteo ? 'text-blue-400/80' : 'text-amber-500/80'} font-bold uppercase tracking-widest mb-1.5 block`}>WhatsApp</label>
                  <input type="text" placeholder="10 dígitos..." value={captura.capTelefono} onChange={(e) => manejarAutocompletado(e.target.value, 'telefono')} className={`w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2.5 text-white outline-none focus:border-${esSorteo ? 'blue' : 'amber'}-500 font-mono text-sm transition-all placeholder:text-slate-600`} />
                </div>
                <div className="relative">
                  <div className="flex justify-between items-center mb-1.5">
                    <label className={`text-[9px] md:text-[10px] ${esSorteo ? 'text-blue-400/80' : 'text-amber-500/80'} font-bold uppercase tracking-widest`}>Nombre del Cliente</label>
                    {captura.capUsuarioId && <button onClick={limpiarCliente} className="text-[9px] text-red-400 hover:text-red-300 uppercase font-black transition-colors">✕ Limpiar</button>}
                  </div>
                  <input type="text" placeholder="Ej. Juan Pérez" value={captura.capNombre} onChange={(e) => manejarAutocompletado(e.target.value, 'nombre')} className={`w-full bg-slate-900 border rounded-lg px-3 py-2.5 text-white outline-none font-bold uppercase text-xs transition-all placeholder:text-slate-600 ${captura.capUsuarioId ? 'border-green-500/50 text-green-400 bg-green-950/20' : `border-slate-700 focus:border-${esSorteo ? 'blue' : 'amber'}-500`}`} />
                </div>
                ...
                {mostrarSugerencias && sugerenciasClientes.length > 0 && (
                  <div className="absolute top-[100%] left-0 w-full bg-slate-800 border border-slate-600 rounded-lg mt-1 z-50 shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2">
                    {sugerenciasClientes.map(cliente => (
                      <div key={cliente.id} onClick={() => aplicarSugerencia(cliente)} className={`px-4 py-2.5 cursor-pointer flex justify-between items-center border-b border-slate-700/50 last:border-0 group transition-colors hover:bg-${esSorteo ? 'blue' : 'amber'}-600`}>
                        <span className="font-bold text-xs uppercase text-slate-300 group-hover:text-white">{cliente.nombre}</span>
                        <span className="font-mono text-[10px] text-slate-500 group-hover:text-slate-200">{cliente.telefono}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {esSorteo ? (
                <div className="bg-blue-950/20 border border-blue-900/50 rounded-xl p-6 text-center shadow-inner mb-5">
                  ...
                  <h4 className="text-blue-400 font-black uppercase tracking-widest text-lg mb-1">Boleto para Sorteo Mundial</h4>
                  <p className="text-slate-400 text-xs font-bold uppercase max-w-md mx-auto">
                    Al guardar este ticket, asegurarás un lugar en la sala. El equipo participante será asignado de forma aleatoria al llegar a los 8 jugadores.
                  </p>
                </div>
              ) : (
                <>
                ...
                {!esSorteo ? (
                  <div className="w-full md:w-[150px] bg-slate-950/40 p-2 rounded-xl border border-slate-800 flex items-center gap-2 transition-all">
                    <label className="text-[8px] md:text-[9px] text-amber-500 font-bold uppercase tracking-widest leading-tight">
                      {esMarcadorExacto ? 'Goles (Auto):' : 'Total Goles:'}
                    </label>
                    <input 
                      type="number" placeholder="00" value={captura.capGoles} onChange={(e) => !esMarcadorExacto && captura.setCapGoles(e.target.value)} readOnly={esMarcadorExacto}
                      className={`w-full border rounded-lg px-2 py-1.5 text-center text-lg font-black outline-none transition-all ${
                        esMarcadorExacto ? 'bg-green-950/20 border-green-500/50 text-green-400 cursor-not-allowed shadow-[0_0_10px_rgba(22,163,74,0.1)]' : 'bg-slate-900 border-slate-700 text-white focus:border-amber-500'
                      }`} 
                    />
                  </div>
                ) : (
                  <div className="w-full md:w-auto text-left">
                     <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Costo del boleto:</span>
                     <p className="text-xl font-black text-blue-400">${captura.quiniela.precio_ticket ?? 1} <span className="text-sm">PESOS</span></p>
                  </div>
                )}
                
                <button 
                  onClick={captura.guardarCapturaFisica} 
                  disabled={captura.guardandoCaptura || (esSorteo && captura.ocupacionSorteo >= 8)} 
                  className={`w-full md:w-auto flex-1 py-3 px-4 rounded-xl font-black uppercase text-[10px] md:text-xs tracking-widest transition-all shadow-lg ${
                    captura.guardandoCaptura 
                      ? 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700' 
                      : (esSorteo && captura.ocupacionSorteo >= 8)
                        ? 'bg-red-900 text-red-300 border-red-700 cursor-not-allowed'
                        : esSorteo 
                          ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-900/50 active:scale-95'
                          : 'bg-amber-500 hover:bg-amber-400 text-slate-900 shadow-amber-900/20 active:scale-95'
                  }`}
                >
                  {captura.guardandoCaptura ? 'Procesando...' : (esSorteo && captura.ocupacionSorteo >= 8) ? 'SALA LLENA (8/8)' : '💾 Cobrar y Guardar'}
                </button>
              </div>
      ...
      <style>{`
        @media print {
          ...
          .zona-impresion { 
            position: absolute !important; left: 0 !important; top: 0 !important; width: 100% !important; height: 100% !important; margin: 0 !important; padding: 15px !important; box-sizing: border-box !important; background-color: white !important;
          }
        }
      `}</style>
        ...
        <div className="hidden print:flex print:flex-col print:items-center print:w-full print:h-full print:bg-white print:text-black zona-impresion z-[99999]">
          <div className="w-full h-full border-4 border-black rounded-3xl p-6 bg-white flex flex-col justify-between">
            <div>
              ...
              <h2 className="text-center font-black text-2xl uppercase mb-6 bg-amber-400 py-2 border-y-4 border-black text-black">
                {captura.quiniela.nombre_jornada}
                {captura.ticketAImprimir.modalidad === 'marcador_exacto' && " (MARCADORES)"}
                {captura.ticketAImprimir.modalidad === 'sorteo' && " (SORTEO MUNDIAL)"}
              </h2>
              ...
              {captura.ticketAImprimir.modalidad === 'sorteo' ? (
                <div className="border-4 border-black border-dashed p-10 text-center rounded-2xl bg-gray-50 mt-6">
                   ...
                   <p className="text-lg font-bold uppercase text-gray-700">Tu equipo será asignado aleatoriamente el día del sorteo.</p>
                   <p className="text-sm font-black mt-6">¡Mucha Suerte!</p>
                </div>
              ) : (
                <>
                  <table className="w-full text-base mb-6 border-collapse table-fixed">
                    <thead>
                      <tr className="bg-blue-900 text-white text-xs uppercase">
                        ...
                        {captura.ticketAImprimir.modalidad === 'marcador_exacto' ? (
                          <th className="border-4 border-black p-2 text-center w-[20%]">Marcador</th>
                        ) : (
                          <>
                    ...
                    <tbody>
                      {(captura.partidos || []).map((p) => {
                        const logoL = obtenerLogo(p.equipo_local)
                        const logoV = obtenerLogo(p.equipo_visitante)
                        const seleccion = captura.ticketAImprimir.selecciones[p.id];
                        ...
                        return (
                          <tr key={p.id}>
                            ...
                            {captura.ticketAImprimir.modalidad === 'marcador_exacto' ? (
                              <td className="border-4 border-black p-1 text-center font-black text-xl text-blue-800 tracking-[0.2em] bg-blue-50">
                                {seleccion || '0-0'}
                              </td>
                            ) : (
                              <>
                                <td className="border-4 border-black p-1 text-center font-black text-xl text-blue-800">{seleccion === 'L' ? 'X' : ''}</td>
                                <td className="border-4 border-black p-1 text-center font-black text-xl text-blue-800">{seleccion === 'E' ? 'X' : ''}</td>
                                <td className="border-4 border-black p-1 text-center font-black text-xl text-blue-800">{seleccion === 'V' ? 'X' : ''}</td>
                              </>
                            )}
